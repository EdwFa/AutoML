...
export class Registration extends Component {
  ...
  changePasswordCheckForm = (e) => {
        this.setState({ formCheckPassword: e.target.value });
  }
  ...
  submitUp() {
    ...
    if (this.state.formCheckPassword != this.state.formPassword) {
        ...
        <div>
          ...
          {token == "" ?
              <form className="space-y-4 md:space-y-6">
                ...
                <div>
                  <label for="password" className="block mb-2 text-sm font-medium text-gray-900 dark:text-white float-start">Ваш пароль повторно</label>
                  <input type="password"
                    name="password"
                    className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-blue-600 focus:border-blue-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"
                    value={formCheckPassword}
                    onChange={this.changePasswordCheckForm}
                    placeholder="••••••••" />
                </div>
                <button type="button"

...
import { variables, GetDatasetParams, showDate, AG_GRID_LOCALE_RU } from "./Variables.js";
...
export class Main extends Component {
  ...
  GetDatasets() {
    fetch(
      variables.API_URL + "main/datasets",
      {
        headers: {
  ...
  onSelectionChanged = () => {
    ...
    this.setState({ dataset: (selectedRows.length === 1 ? selectedRows[0] : null) })
  }
  ...
  LoadDataset(dataset) {
    fetch(
      ...
      .catch((error) => {
        console.log(error);
        this.setState({ datasetRows: [], datasetColumns: [], countRows: 0, countColumns: 0 });
      });
  }
  ...
  changeGroup = (e) => {
    ...
    this.setState({ group_data: this.state.datasetRows.map((el) => el[e.field]) });
  };
  ...
  changeLabel = (e) => {
    this.setState({
        LearnLabel: e,
        NumberLabels: this.state.NumberLabels.filter(x=> x.id !== e.id).map(x => x),
        CategoricalLabels: this.state.CategoricalLabels.filter(x=> x.id !== e.id).map(x => x)
    });
  };
  ...
  changeNumberLabels = (e) => {
    this.setState({
        ...
        LearnLabel: this.state.LearnLabel? e.map((label) => label.id).includes(this.state.LearnLabel.id) ? null : this.state.LearnLabel : null,
        CategoricalLabels: this.state.CategoricalLabels.filter(x=> !(e.map((label) => label.id).includes(x.id))).map(x => x)
    });
  };
  ...
  changeCategoricalLabels = (e) => {
    this.setState({
        ...
        LearnLabel: this.state.LearnLabel? e.map((label) => label.id).includes(this.state.LearnLabel.id) ? null : this.state.LearnLabel : null,
        NumberLabels: this.state.NumberLabels.filter(x=> !(e.map((label) => label.id).includes(x.id))).map(x => x),
    });
  };
    ...
    fetch(
        ...
        `?datasetId=${dataset.id}`,
      {
        ...
        body: JSON.stringify({
          ...
          categorical_columns: this.state.CategoricalLabels.map((label) => label.name),
              ...
              <Tab.Panels className="">
                ...
                >
                  ...
                  <div>
                    ...
                    <div className="mt-4 flex text-sm leading-6 text-gray-600" style={{height: 300, width: 1000}}>
                        <div
                ...
                >
                  ...
                  {datasetRows.length !== 0 ? (
                    <div className="mb-4 block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                      ...
                      <Select
                        ...
                        isClearable
                      />
                      <label for="date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Категориальные данные</label>
                      <Select
                        ...
                        isMulti
                      />
                      <label for="date" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Числовые данные</label>
                      <Select
                        className='basic-multi-select'
                        classNamePrefix='select'
                        options={labels}
                  ...
                  {LearnInfo ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 gap-y-4">
                      ...
                      <div className="mb-4 block p-6 bg-white border border-gray-200 rounded-lg shadow hover:bg-gray-100 dark:bg-gray-800 dark:border-gray-700 dark:hover:bg-gray-700">
                        <div className="ag-theme-alpine ag-theme-acmecorp" style={{height: "100%", width: "100%"}}>
                                            <AgGridReact
